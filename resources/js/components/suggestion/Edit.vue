    ...
    <div class="col-md-6 justify-content-center m-auto">
      <div class="card shadow">
        ...
        <div class="card-header border-0 tag-title d-flex justify-content-between align-items-center">
          	...
          	<h3 class="mb-0"> {{ $t('back_end.table.edit_result') }} </h3>
            ...
            <div class="row">
              <div class="col-md-12">
                <div class="form-group">
                  ...
                  <select class="form-control" :class="{ 'is-invalid': form.errors.has('icon_id') }" v-model="form.icon_id">
                    <option disabled value="">Select One</option>
                    <option :value="icon.id" v-for="icon in icons.icons" :key="icon.id">{{ icon.title }}</option>
                  </select>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            createSuggestion(){
                this.$Progress.start();
                this.form.patch(`/api/suggestions/${this.$route.params.id}`)
                .then(()=>{
            ...
            loadIcon(){
                axios.get(`/api/edit-suggestion/${this.$route.params.id}`).then(({ data }) => (this.icons = data));
            },
        ...
        mounted() {
           this.loadIcon();
            axios.get(`/api/edit-suggestion/${this.$route.params.id}`)
              .then((response)=>{
                this.form.fill(response.data.suggestion);
            })
        }
    }
